/**
 * Copyright (c) 2011 Red Hat, Inc. <http://www.redhat.com> This file is part of GlusterFS.
...
public class GlusterVolume extends RawLocalFileSystem {
    ...
    public boolean sameVolume(Path p) {
        ...
        URI thatUri = p.toUri();
        if (!thisUri.getScheme().equalsIgnoreCase(thatUri.getScheme())) {
            return false;
        }
        if ((thisUri.getAuthority() == null && thatUri.getAuthority() == null)) {
            return true;
        }
    ...
    public void setConf(Configuration conf) {
        ...
        if (conf != null) {
            ...
            try {
                ...
                default_volume = v[0];
                for (String s : v) {
                    String vol = conf.get("fs.glusterfs.volume.fuse." + s, null);
                    
                    if (vol == null) {
                        log.error("Invalid volume name: " + s
                                  + ", No such property: fs.glusterfs.fuse." + s);
                        throw new RuntimeException("Invalid volume name: " + s
                                                   + ", No mount point available for the volume.");
                    }
                    volumes.put(s, vol);
                    log.info("Gluster volume: " + s + " at : " + volumes.get(s));
                }
                 ...
                 * Write Buffering
                 */
                int userBufferSize = conf.getInt("io.file.buffer.size", -1);
    ...
    public boolean rename(Path src, Path dst) throws IOException {
        ...
        if (dest.exists() && dest.isFile() || !(new File(dest.getParent()).exists())) {
            return false;
        }
    ...
    public FileStatus[] listStatus(Path f) throws IOException {
        ...
        for (File name : names) {
            try {
                FileStatus listing = getFileStatus(fileToPath(name));
                ...
                log.info("ignoring invisible path :  " + name);
            }
        }
    ...
    public BlockLocation[] getFileBlockLocations(FileStatus file, long start, long len) throws IOException {
        if (start < 0){
            throw new IllegalArgumentException("Start '"+start+"' negative");
        }
        if (len < 0){
            throw new IllegalArgumentException("Len '"+len+"' negative");
        }
        if (file == null) {
            return null;
